...
class ApiCalls(object):
    ...
    # Instance of __ApiCalls, ApiCalls acts as a singleton that can be reinitialized
    Instance = None

    def __init__(self, client_id, client_secret, base_URL, username, password, max_wait_time=20):

        if not ApiCalls.Instance or ApiCalls.Instance.parameters_are_different(client_id, client_secret, base_URL,
                                                                               username, password, max_wait_time):
            logging.debug("ApiCalls object does not exist or was given new parameters, creating a fresh instance.")
            ApiCalls.Instance = ApiCalls.__ApiCalls(client_id, client_secret, base_URL,
                                                    username, password, max_wait_time)
        else:
            logging.debug("ApiCalls object already exists, reusing.")


    class __ApiCalls(object):
        ...
        def parameters_are_different(self, client_id, client_secret, base_URL, username, password, max_wait_time):
            if (self.client_id != client_id or
