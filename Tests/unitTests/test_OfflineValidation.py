...
from Validation.offlineValidation import (validateSampleSheet,
                                          validatePairFiles,
                                          validateSampleList, validateURLForm)
...
class TestOfflineValidation(unittest.TestCase):
    ...
    def test_validateSampleSheet_emptySheet(self):
        csvFile = path.join(
            pathToModule, "testSampleSheets", "emptySampleSheet.csv")
        vRes = validateSampleSheet(csvFile)
        ...
        self.assertTrue(
            ...
            " Description, Sample_ID" in vRes.getErrors())
        self.assertTrue(
            "[Header] section not found in SampleSheet" in vRes.getErrors())
        self.assertTrue(
            "[Data] section not found in SampleSheet" in vRes.getErrors())

    def test_validateSampleSheet_missing_DataHeader(self):
        # has [Header]+[Data] but missing required data header (Sample_Project)
        csvFile = path.join(
            pathToModule, "testSampleSheets", "missingDataHeader.csv")
        vRes = validateSampleSheet(csvFile)
        self.assertFalse(vRes.isValid())
    ...
    def test_validateSampleSheet_missing_HeaderSection(self):
        ...
        csvFile = path.join(
            pathToModule, "testSampleSheets", "missingHeaderSection.csv")
        vRes = validateSampleSheet(csvFile)
        self.assertFalse(vRes.isValid())
        self.assertEqual(vRes.errorCount(), 1)
    ...
    def test_validatePairFiles_invalid_seqFiles(self):
        dataDir = path.join(
            pathToModule, "testSeqPairFiles", "invalidSeqFiles")
    ...
    def test_validateSampleList_invalid_noSampleProj(self):
        ...
        csvFile = path.join(
            pathToModule, "testSeqPairFiles", "noSampleProj",
            "SampleSheet.csv")
        ...
        self.assertEqual(vRes.errorCount(), 1)
        self.assertTrue(
            "No sampleProject found for sample" in vRes.getErrors())
    ...
    def test_validateSampleList_invalid_Empty(self):
        ...
        self.assertEqual(vRes.errorCount(), 1)
        self.assertTrue(
            "The given list of samples is empty" in vRes.getErrors())
